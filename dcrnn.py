...
class RecurrentGCN_DCRNN(torch.nn.Module):
    def __init__(self, node_features):
        ...
        self.recurrent = DCRNN(node_features, node_features, 2)
...
def test_dcrnn(time_steps, criterion, plot):
    ...
    model = RecurrentGCN_DCRNN(feature_dim)
    optimizer = torch.optim.Adam(model.parameters(), lr=0.01)

    train_model.train_single_shot(model, optimizer, dynamic_node_features_train, static_edge_index, static_edge_weight, criterion, 180)

    pred, lab = eval_model.eval_single_shot(model, dynamic_node_features_train, static_edge_index, static_edge_weight, criterion)
